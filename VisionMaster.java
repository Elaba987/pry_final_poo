...
public class VisionMaster {
    ...
    static Cliente seleccionarOCrearCliente() {
        ...
        if (opc == 1) return elegirClienteDeLista();
        if (opc == 2) return crearCliente();
        return null;
    }
    ...
    static Cliente elegirClienteDeLista() {
        ...
        if (idx >= 0 && idx < clientes.size()) return clientes.get(idx);
        return null;
    }
    ...
    static void configurarProducto(OrdenCompra orden) {
        ...
        while (opc != 3) {
            ...
            if (opc == 1) elegirArmazon(orden);
            else if (opc == 2) elegirAccesorio(orden);
        }
    ...
    static void elegirArmazon(OrdenCompra orden) {
        // Filtrar armazones - instanceof como en Mascotas.java
        List<Producto> armazones = new ArrayList<>();
        for (Producto p : tienda.getInventario()) {
            if (p instanceof Armazon) armazones.add(p);
        }

        System.out.println("\n-- Armazones --");
        for (int i = 0; i < armazones.size(); i++) {
            System.out.println((i + 1) + ". " + armazones.get(i));
        }
        System.out.println("0. Volver al menu anterior");
        System.out.print("Elige numero: ");
        int idx = leerEntero() - 1;
        if (idx < 0 || idx >= armazones.size()) return;

        Armazon base = (Armazon) armazones.get(idx);   // Cast como en Mascotas.java

        // Elegir graduacion con enum - como DiaFavorito2.java
        ...
        if (tIdx < 0 || tIdx >= tipos.length) return;
    ...
    static void elegirAccesorio(OrdenCompra orden) {
        ...
        for (Producto p : tienda.getInventario()) {
            if (p instanceof Accesorio) accesorios.add(p);
        }
        ...
        System.out.print("Elige numero: ");
        int idx = leerEntero() - 1;
        if (idx < 0 || idx >= accesorios.size()) return;
    ...
    // =====================================================================
    // MENU ADMINISTRAR CLIENTES (CRUD)
    // =====================================================================
    ...
    static void verListaClientes() {
        ...
        if (ordenOpc == 2)      comparador = new ComparadorClienteIdDesc();
        else if (ordenOpc == 3) comparador = new ComparadorClienteNombreAZ();
        else if (ordenOpc == 4) comparador = new ComparadorClienteNombreZA();
        else                    comparador = new ComparadorClienteIdAsc();
    ...
    static void editarCliente() {
        ...
        if (opc == 0) return;
        if (opc == 1 || opc == 4) { System.out.print("Nuevo nombre (" + c.getNombre() + "): ");    c.setNombre(sc.nextLine()); }
        if (opc == 2 || opc == 4) { System.out.print("Nuevo telefono (" + c.getTelefono() + "): "); c.setTelefono(sc.nextLine()); }
        if (opc == 3 || opc == 4) { System.out.print("Nuevo correo (" + c.getCorreo() + "): ");    c.setCorreo(sc.nextLine()); }
    ...
    static void verInventario() {
        ...
        if (filtroOpc == 0) return;

        String filtro;
        if (filtroOpc == 1) filtro = "armazones";
        else if (filtroOpc == 2) filtro = "accesorios";
        else filtro = "todos";
        ...
        if (ordenOpc == 1)      comparador = new ComparadorNombreAZ();
        else if (ordenOpc == 2) comparador = new ComparadorNombreZA();
        else if (ordenOpc == 4) comparador = new ComparadorPrecioDesc();
        else                    comparador = new ComparadorPrecio();

        List<Producto> lista = tienda.getListaOrdenada(filtro, comparador);
        if (lista.isEmpty()) { System.out.println("Sin productos en esta seccion."); return; }
    ...
    static void agregarProducto() {
        ...
        if (tipo == 1) {
            ...
            if (mIdx < 0 || mIdx >= materiales.length) return;
    ...
    static void editarProducto() {
        ...
        if (inv.isEmpty()) { System.out.println("Inventario vacio."); return; }
        ...
        for (Producto p : inv) {
            if (filtroOpc == 1 && p instanceof Armazon) lista.add(p);
            else if (filtroOpc == 2 && p instanceof Accesorio) lista.add(p);
            else if (filtroOpc == 3) lista.add(p);
        }

        if (lista.isEmpty()) { System.out.println("Sin productos en esta seccion."); return; }

        for (int i = 0; i < lista.size(); i++) {
            if (lista.get(i) instanceof Armazon) System.out.println((i + 1) + ". " + ((Armazon) lista.get(i)).toStringCompleto());
            else System.out.println((i + 1) + ". " + lista.get(i));
        }
        System.out.println("0. Volver");
        System.out.print("Elige numero: ");
        int idx = leerEntero() - 1;
        ...
        if (p instanceof Armazon) editarArmazon((Armazon) p);
        else if (p instanceof Accesorio) editarAccesorio((Accesorio) p);
    }
    ...
    static void editarArmazon(Armazon a) {
        ...
        if (opc == 1 || opc == 5) { System.out.print("Nuevo nombre (" + a.getNombre() + "): ");  a.setNombre(sc.nextLine()); }
        if (opc == 2 || opc == 5) { System.out.print("Nuevo precio (" + a.getPrecio() + "): ");  a.setPrecio(Double.parseDouble(sc.nextLine())); }
        if (opc == 3 || opc == 5) {
            MaterialLente[] materiales = MaterialLente.values();
            for (int i = 0; i < materiales.length; i++) System.out.println((i + 1) + ". " + materiales[i].getNombre());
            System.out.print("Nuevo material: ");
            int mIdx = leerEntero() - 1;
            if (mIdx >= 0 && mIdx < materiales.length) a.setMaterial(materiales[mIdx]);
        }
        if (opc == 4 || opc == 5) { System.out.print("Nuevo estilo (" + a.getEstilo() + "): ");  a.setEstilo(sc.nextLine()); }
    ...
    static void editarAccesorio(Accesorio a) {
        ...
        if (opc == 1 || opc == 4) { System.out.print("Nuevo nombre (" + a.getNombre() + "): ");  a.setNombre(sc.nextLine()); }
        if (opc == 2 || opc == 4) { System.out.print("Nuevo precio (" + a.getPrecio() + "): ");  a.setPrecio(Double.parseDouble(sc.nextLine())); }
        if (opc == 3 || opc == 4) { System.out.print("Nuevo tipo (" + a.getTipo() + "): ");      a.setTipo(sc.nextLine()); }
    ...
    static void eliminarProducto() {
        ...
        for (int i = 0; i < inv.size(); i++) {
            if (inv.get(i) instanceof Armazon) System.out.println((i + 1) + ". " + ((Armazon) inv.get(i)).toStringCompleto());
            else System.out.println((i + 1) + ". " + inv.get(i));
        }
        System.out.println("0. Volver");
        System.out.print("Elige numero: ");
        int idx = leerEntero() - 1;
